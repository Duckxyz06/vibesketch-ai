...
const App: React.FC = () => {
  // API Key: true if build-time env OR localStorage
  const hasEnvKey = !!(process.env.API_KEY || process.env.GEMINI_API_KEY);
  const [hasApiKey, setHasApiKey] = useState(
    hasEnvKey || !!localStorage.getItem('vibesketch_gemini_key')
  );
  ...
  const handleClearKey = () => {
    localStorage.removeItem('vibesketch_gemini_key');
    setHasApiKey(false);
  };
  ...
  const renderContent = () => {
    if (!hasApiKey) {
      return <ApiKeyGate onKeySet={() => setHasApiKey(true)} />;
    }
    ...
    <div className="min-h-screen paper-texture flex flex-col">
      <header className="w-full p-6 border-b-2 border-gray-200/50 flex justify-between items-center bg-white/60 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2 md:gap-4">
             ...
             {!hasApiKey && (
                 <Button variant="secondary" onClick={() => setHasApiKey(false)} className="text-sm px-4 py-1">
                     Nhập Key
                 </Button>
             )}
        </div>
